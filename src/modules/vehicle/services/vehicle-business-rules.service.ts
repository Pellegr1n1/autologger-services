...
export class VehicleBusinessRulesService {
  ...
  async validateActiveVehicleLimit(userId: string): Promise<void> {
    const activeVehiclesCount =
      await this.vehicleRepository.countActiveByUserId(userId);
  ...
  async validateUniquePlate(plate: string, excludeId?: string): Promise<void> {
    ...
    if (exists) {
      throw new ConflictException(
        `Já existe um veículo cadastrado com a placa ${plate}`,
      );
    }
  }
  ...
  async validateVehicleOwnership(
    ...
    const vehicle = await this.vehicleRepository.findByIdAndUserId(
      vehicleId,
      userId,
    );
  ...
  async validateVehicleCanBeSold(
    vehicleId: string,
    userId: string,
  ): Promise<void> {
    const vehicle = await this.vehicleRepository.findByIdAndUserId(
